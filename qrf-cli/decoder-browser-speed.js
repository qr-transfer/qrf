...
class BrowserSpeedDecoder {
  ...
  addChunkToFile(fileName, fileInfo, packet, timestamp) {
    ...
    try {
      ...
      // Show progress every 20 chunks for less console spam
  ...
  showProgress() {
    const now = Date.now();
    if (now - this.lastProgressUpdate < 100) return; // Update every 100ms
...
// Browser-speed video processor using Sharp + jsQR (same as browser)
async function processVideoBrowserSpeed(videoPath, fps, decoder) {
  return new Promise((resolve, reject) => {
    let frameId = 0;

    // Optimized FFmpeg args for browser-like speed
    const args = [
      '-i', videoPath,
      '-threads', '0',
      '-vf', `fps=${fps}`,
      '-c:v', 'mjpeg',
      '-q:v', '2', // High quality for better QR detection
      '-f', 'image2pipe',
      '-'
    ];
    ...
    // Process frame using Sharp + jsQR (same stack as browser)
    const processFrameBrowserSpeed = async (frameData, id) => {
      try {
        // Use Sharp (MUCH faster than Jimp) to get RGBA data
        const { data, info } = await sharp(frameData)
          .raw()
          .ensureAlpha()
          .toBuffer({ resolveWithObject: true });

        // Use jsQR (same library as browser) for QR detection
        const code = jsQR(data, info.width, info.height);

        if (code && code.data) {
          await decoder.processQRData(code.data, id / fps);
        }
      } catch (error) {
    ...
    ffmpegProcess.stdout.on('data', async (chunk) => {
      ...
      while (true) {
        ...
        // Process frame immediately using browser-speed libraries
        await processFrameBrowserSpeed(frame, frameId);
...
async function decodeVideo(videoPath, options) {
  const fps = options.fps || 25; // Higher default
  const outputDir = options.output || './decoded';

  console.log('\n🎬 QRF Browser-Speed Decoder\n');
  console.log(`📹 Video: ${videoPath}`);
  console.log(`⚡ Scan rate: ${fps} FPS (BROWSER SPEED)`);
  console.log(`📁 Output: ${outputDir}`);
  console.log(`✅ Using Sharp + jsQR (same as browser)\n`);
  ...
  // Initialize decoder
  const decoder = new BrowserSpeedDecoder();

  console.log('🔥 Processing video at BROWSER SPEED (Sharp + jsQR)...\n');

  // Process video
  await processVideoBrowserSpeed(videoPath, fps, decoder);
...
if (process.argv.length < 3) {
  console.log('Usage: node decoder-browser-speed.js <video> [options]');
  console.log('Options:');
  console.log('  --fps <rate>     Frame extraction rate (default: 25)');
  console.log('  --output <dir>   Output directory (default: ./decoded)');
  console.log('\nBrowser-speed decoder using Sharp + jsQR (same libraries as browser)');
  process.exit(1);
}
