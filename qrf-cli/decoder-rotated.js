...
class RotationAwareDecoder {
  ...
  showProgress() {
    const now = Date.now();
    if (now - this.lastProgressUpdate < 500) return; // Update every 500ms
...
// Rotation-aware video processor with 180-degree correction
async function processVideoWithRotation(videoPath, fps, decoder) {
  return new Promise((resolve, reject) => {
    ...
    // Process frame with multiple rotation attempts for QR detection
    const processFrameWithRotation = async (frameData, id) => {
      try {
        // First try: normal orientation (already rotated by ffmpeg)
        let { data, info } = await sharp(frameData)
          .raw()
          .ensureAlpha()
        ...
        // If no QR found, try additional rotations
        if (!qrResult) {
          // Try 90 degree rotation
          const rotated90 = await sharp(frameData)
            .rotate(90)
            .raw()
            .ensureAlpha()
            .toBuffer({ resolveWithObject: true });

          qrResult = jsQR(rotated90.data, rotated90.info.width, rotated90.info.height);

          if (!qrResult) {
            // Try 270 degree rotation
            const rotated270 = await sharp(frameData)
              .rotate(270)
              .raw()
              .ensureAlpha()
              .toBuffer({ resolveWithObject: true });

            qrResult = jsQR(rotated270.data, rotated270.info.width, rotated270.info.height);
          }
        }

        if (qrResult && qrResult.data) {
          await decoder.processQRData(qrResult.data, id / fps);
        }
      } catch (error) {
    ...
    ffmpegProcess.stdout.on('data', async (chunk) => {
      ...
      while (true) {
        ...
        // Process frame with rotation handling
        await processFrameWithRotation(frame, frameId);
...
async function decodeVideo(videoPath, options) {
  ...
  console.log('\n🎬 QRF Rotation-Aware Decoder\n');
  console.log(`📹 Video: ${videoPath}`);
  console.log(`⚡ Scan rate: ${fps} FPS`);
  console.log(`📁 Output: ${outputDir}`);
  console.log(`🔄 Auto-rotation enabled (handles rotated videos)\n`);
  ...
  const hasRotation = metadata.streams[0].side_data?.find(data =>
    data.side_data_type === 'Display Matrix'
  );

  if (hasRotation) {
    console.log(`📊 Video has rotation metadata - auto-correcting`);
  }
  ...
  // Initialize decoder
  const decoder = new RotationAwareDecoder();

  console.log('🔥 Processing video with rotation correction...\n');

  // Process video
  await processVideoWithRotation(videoPath, fps, decoder);
...
if (process.argv.length < 3) {
  console.log('Usage: node decoder-rotated.js <video> [options]');
  console.log('Options:');
  console.log('  --fps <rate>     Frame extraction rate (default: 20)');
  console.log('  --output <dir>   Output directory (default: ./decoded)');
  console.log('\nHandles rotated videos automatically (180/90/270 degree rotation)');
  process.exit(1);
}
